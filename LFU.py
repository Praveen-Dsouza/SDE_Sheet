...
class LFUCache:
    ...
    def put(self, key: int, value: int) -> None:
        ...
        if key in self.cache: self.updateCache(self.cache[key], key, value)
        else:
    ...
    def updateCache(self, node, key, value):
        ...
        prevFreq = node.freq
        node.freq += 1
        self.freqTable[prevFreq].removeNode(node) # remove node
        self.freqTable[node.freq].insertHead(node) # insert with updated freq
